...
export default function SubmitInsightPage() {
  ...
  const selectedBidderObj = bidders.find((b) => String(b.no) === selectedBidder) || null;
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch('/api/insights', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          bidderNo: selectedBidderObj?.no || null,
          bidderName: selectedBidderObj?.name || '',
          updateType,
          rawNotes,
          csInsight: draft,
        ...
        {step === 1 && (
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6 space-y-5">
            ...
            <div>
              ...
              >
                <option value="">{loadingBidders ? 'Loading…' : '— Select bidder —'}</option>
                {bidders.map((b) => (
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Paste your raw notes, meeting summary, or update here…"
            ...
            <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6 space-y-4">
              ...
              {selectedBidderObj && (
                <div className="text-xs text-gray-500">
                  <span className="font-medium text-gray-700">{selectedBidderObj.name}</span>
                  {selectedBidderObj.tier && <span> · Tier {selectedBidderObj.tier}</span>}
                  {analystName && <span> · {analystName}</span>}
                  {infoDate && <span> · {new Date(infoDate).toLocaleDateString('en-AU', { day: 'numeric', month: 'short', year: 'numeric' })}</span>}
                </div>
              )}
          ...
          <div className="bg-white border border-emerald-200 rounded-xl shadow-sm p-8 text-center space-y-5">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {savedInsight.bidder_name || 'Bidder'} · {savedInsight.update_type}
              </p>
            </div>
