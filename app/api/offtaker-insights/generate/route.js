...
export async function POST(req) {
  const { offtakerName, updateType, rawNotes } = await req.json();
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    const placeholder =
      `[API key not configured] Draft insight for ${offtakerName || 'this offtaker'}: ` +
  ...
  const userPrompt = [
    ...
    offtakerName ? `offtaker: ${offtakerName}` : '',
    '',
    'Raw notes:',
    rawNotes.trim(),
  ]
