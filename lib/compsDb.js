...
export async function ensureCompsSchema() {
  ...
  // Best-effort unique index on name so additive sync is race-safe (ON CONFLICT).
  try { await sql`CREATE UNIQUE INDEX IF NOT EXISTS comp_deals_name_uidx ON comp_deals (name)`; } catch { /* legacy duplicate names — skip */ }
}

// Insert one deal (+ its metrics). additive=true uses ON CONFLICT (name) DO
// NOTHING so concurrent loads don't double-insert; returns the new id, or null
// when the name already existed.
async function insertOneDeal(d, additive) {
  const rows = await sql`
    ...
    ${additive ? sql`ON CONFLICT (name) DO NOTHING` : sql``}
    RETURNING id
  `;
  const id = rows[0]?.id;
  ...
  for (const m of d.metrics || []) {
    await sql`
      ...
      VALUES (${id}, ${m.category || categoryForMetric(m.metric)}, ${m.metric}, ${m.value ?? null},
...
// Auto-sync on page load: add any dataset deals missing from the DB (by name).
// Non-destructive — preserves UI-added deals and any UI edits to existing rows.
export async function syncDealsAdditive() {
  const existing = new Set((await sql`SELECT name FROM comp_deals`).map((r) => r.name));
  let n = 0;
  for (const d of SEED_DEALS) {
    if (existing.has(d.name)) continue;
    if (await insertOneDeal(d, true)) n++;
  }
  return n;
}

// Manual "Resync from dataset": refresh every dataset deal from the JSON
// (delete same-name + re-insert, picking up changed values). Leaves UI-added
// deals (not in the dataset) untouched.
export async function resyncDealsFull() {
  let n = 0;
  for (const d of SEED_DEALS) {
    await sql`DELETE FROM comp_deals WHERE name = ${d.name}`;
    await insertOneDeal(d, false);
    n++;
  }
  return n;
}
