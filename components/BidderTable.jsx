...
const COLUMNS = [
  ...
  { key: 'parentOwner', label: 'Parent', w: 'w-20' },
  { key: 'geography', label: 'Geo', w: 'w-14' },
  { key: 'tier', label: 'T', w: 'w-6' },
  { key: 'type', label: 'Ty', w: 'w-10' },
  { key: 'csInsights', label: 'CS Insights', w: 'w-32' },
  { key: 'aiInsights', label: 'AI Insights', w: 'w-44' },
  { key: 'mergedInsights', label: 'Merged Insights', w: 'w-48' },
  { key: 'contact', label: 'Contact', w: 'w-20' },
  { key: 'email', label: 'Email', w: 'w-24' },
  { key: 'phone', label: 'Phone', w: 'w-16' },
  { key: 'aiScore', label: 'Score', w: 'w-10' },
];
...
function tierBadge(tier) {
  ...
  return (
    <span className={`inline-block px-1 py-0.5 rounded-full text-[9px] font-semibold ${colors[tier] || 'bg-gray-100'}`}>
      {tier}
    </span>
  );
}
...
function typeBadge(type) {
  ...
  return (
    <span className={`inline-block px-1 py-0.5 rounded-full text-[9px] font-semibold ${isStrat ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'}`}>
      {isStrat ? 'S' : 'F'}
    </span>
  );
}
...
function aiScoreBadge(score, label, reason) {
  ...
  return (
    <span
      title={reason}
      className={`inline-flex items-center justify-center w-6 h-6 rounded-md text-[10px] font-bold cursor-help ${colors[label] || 'bg-gray-300'}`}
    >
      {label}
    </span>
  );
    ...
    return (
      <div className="space-y-1">
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={() => onReject(bidder.no)} className="text-[9px] text-gray-400 hover:text-red-500 underline">rej</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-1">
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={() => onAccept(bidder.no)} className="text-[9px] text-gray-400 hover:text-green-600 underline">acc</button>
        </div>
      </div>
    );
  }
          ...
          <tr>
            ...
            {COLUMNS.map((col) => (
              <th
                ...
                className={`px-1 py-1 text-left font-semibold text-gray-600 select-none whitespace-nowrap text-[10px] ${col.w} ${col.key === 'aiScore' ? 'text-center' : ''} ${col.key === 'mergedInsights' ? '' : 'cursor-pointer hover:bg-gray-200'}`}
              >
                {col.label}
                {sortKey === col.key && (
              ...
              {COLUMNS.map((col) => (
                <td
                  ...
                  className={`px-1 py-0.5 ${
                    col.key === 'mergedInsights' ? 'whitespace-normal min-w-[180px] max-w-[280px]' :
                    col.key === 'aiInsights' ? 'whitespace-normal min-w-[160px] max-w-[260px]' :
                    col.key === 'csInsights' ? 'whitespace-normal max-w-[160px]' :
                    col.key === 'name' || col.key === 'parentOwner' ? 'whitespace-nowrap truncate max-w-[100px]' :
                    col.key === 'geography' ? 'whitespace-nowrap truncate max-w-[70px]' :
                    col.key === 'aiScore' ? 'text-center' :
                    'whitespace-nowrap truncate max-w-[120px]'
                  }`}
                  title={col.key === 'name' || col.key === 'parentOwner' || col.key === 'geography' ? (b[col.key] ?? '') : undefined}
                >
                  {col.key === 'tier' ? tierBadge(b.tier) :
                   ...
                   col.key === 'mergedInsights' ? (
                     <MergedInsightCell bidder={b} />
                   ) :
                   col.key === 'aiInsights' ? (
                     <AiInsightCell
                       bidder={b}
                   ...
                   col.key === 'csInsights' ? (
                     <span className="text-[10px] leading-tight line-clamp-2">{b.csInsights}</span>
                   ) :
                   (b[col.key] ?? '')}
                </td>
              ))}
