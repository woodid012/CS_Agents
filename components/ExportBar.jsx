...
const EXPORT_COLUMNS = [
  { key: 'no', label: '#' },
  { key: 'name', label: 'Name' },
  { key: 'parentOwner', label: 'Parent/Owner' },
...
function buildFilename(txName, ext) {
  ...
  return `${prefix}bidder_list.${ext}`;
}

export default function ExportBar({ filteredBidders, allBidders, selected, insightStatus }) {
  ...
  const count = selected.size > 0 ? selected.size : filteredBidders.length;
